...
function isInBlockStatement(line) {
    for (let i = blocks.length - 1; i > 0; i--) {
        if (blocks[i].start <= line && blocks[i].end >= line) {
            return i;
        }
    }
}
...
function updateLastBeforeBlock() {
    for (let i = blocks.length - 1; i > 0; i--) {
        let j = i;
        while (blocks[j].start > blocks[j - 1].end) {
            j--;
        }
        if (line_numbers.includes(blocks[j].start - 1)) {
            ...
            blocks[i].lastBeforeBlock = getCurrectLineNum(blocks[j].start - 1);
        }
    }
...
function insertLocalsInitiateValue(i) {
    if (nodes_after_parse[i].Line < func_declaration_line) {
        ...
        local_vars_map.set(nodes_after_parse[i].Name, '');
    }
}
...
function insert_line_dic(idx, line) {
    ...
    for (let i = 0; i < exp.length; i++) {
        if (localDic.has(exp[i]) && localDic[nodes_after_parse[i].Name] != '') {
...
function updateBlockConditionValue(lineNum,condition){
    ...
    for(let i=0;i<exp.length;i++){
        let varName=exp[i];
        if(localDic.has(varName) && localDic.get(varName)!='' && !isValueInParamArray(varName)){
...
function insertLinesToPrint(){
    ...
    let linesToPrint=[];
    for(let i=0;i<nodes_after_parse.length;i++){
        if( globalsName.includes(nodes_after_parse[i].Name)|| ifBlocksContains(nodes_after_parse[i].Line)){ //if global or blockstatement
...
export {parseCode};
export {assignment2};
export {insertLinesToPrint};
export {returnLinesDic};
export {getParamsNameArray};
export {textAfterParse};
